...
const bool MasterRenderer::TEST_FRUSTUM_CULLING = false;
...
void MasterRenderer::prepare(std::vector<Terrain*> &terrainList, std::vector<Entity*> &entityList)
{
	//Source: http://www.racer.nl/reference/vfc_markmorley.htm
	{
		...
		frustum[0].normal = glm::vec3(vpMatrix[0].w - vpMatrix[0].x, vpMatrix[1].w - vpMatrix[1].x, vpMatrix[2].w - vpMatrix[2].x);
		frustum[0].distance = vpMatrix[3].w - vpMatrix[3].x;
		// Left clipping plane.
		frustum[1].normal = glm::vec3(vpMatrix[0].w + vpMatrix[0].x, vpMatrix[1].w + vpMatrix[1].x, vpMatrix[2].w + vpMatrix[2].x);
		frustum[1].distance = vpMatrix[3].w + vpMatrix[3].x;
		// Bottom clipping plane.
		frustum[2].normal = glm::vec3(vpMatrix[0].w + vpMatrix[0].y, vpMatrix[1].w + vpMatrix[1].y, vpMatrix[2].w + vpMatrix[2].y);
		frustum[2].distance = vpMatrix[3].w + vpMatrix[3].y;
		// Top clipping plane.
		frustum[3].normal = glm::vec3(vpMatrix[0].w - vpMatrix[0].y, vpMatrix[1].w - vpMatrix[1].y, vpMatrix[2].w - vpMatrix[2].y);
		frustum[3].distance = vpMatrix[3].w - vpMatrix[3].y;
		// Far clipping plane.
		frustum[4].normal = glm::vec3(vpMatrix[0].w - vpMatrix[0].z, vpMatrix[1].w - vpMatrix[1].z, vpMatrix[2].w - vpMatrix[2].z);
		frustum[4].distance = vpMatrix[3].w - vpMatrix[3].z;
		// Near clipping plane.
		frustum[5].normal = glm::vec3(vpMatrix[0].w + vpMatrix[0].z, vpMatrix[1].w + vpMatrix[1].z, vpMatrix[2].w + vpMatrix[2].z);
		frustum[5].distance = vpMatrix[3].w + vpMatrix[3].z;

		// Normalize, this is not always necessary...
		for (unsigned int i = 0; i < 6; i++)
		{
			float normFact = glm::length(frustum[i].normal);
			frustum[i].distance = frustum[i].distance / normFact;
			frustum[i].normal = frustum[i].normal / normFact;
		}
	}
...
void MasterRenderer::processEntity(Entity* entity)
{
	...
	if (entity->staticEntity)
	{
		transformedAABB = entity->aabb;
	}
	else
	{
		entity->model->getRawModel()->aabb.getTransformed(entity->modelMatrix, transformedAABB);
	}
	if (RenderController::viewfrustumCullingDisabled || isAABBinFrustum(transformedAABB))
	{
...
{
	if (RenderController::viewfrustumCullingDisabled || isAABBinFrustum(terrain->aabb))
...
//Source: http://www.racer.nl/reference/vfc_markmorley.htm
bool MasterRenderer::isAABBinFrustum(const AABB &aabb)
{
	//short result = 2;
	for (int i = 0; i < 6; i++)
	{
